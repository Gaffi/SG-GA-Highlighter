...
window.onload = function() {
	...
	if (window.location.href.match(".steamgifts.com/") !== null &&
		...
		for(i=0; i < header_entries.length; i++) {
		...
		for(i=0; i < heading_entries.length; i++) {
		...
		for (var e=0;e<entries.length;e++){
			...
			var copies = null;
			for (i = 0; i < entries[e].childNodes.length; i++) {
				if (entries[e].childNodes[i].className == "giveaway__heading") {
					for (j=0; j<entries[e].childNodes[i].childNodes.length;j++) {
